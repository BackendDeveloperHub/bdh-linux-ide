...
void draw_ui() {
    ...
    // 4. Right Pane (EDITOR & LINE NUMBERS)
    if (focus == 0 || focus == 1 || focus == 3) {
        ...
        int row = 3;
        int line_num = 1;
        int offset = 6; // லைன் நம்பருக்கான இடைவெளி
        ...
        // 🔥 Custom line parser to support empty lines & Line Numbers
        while (row <= total_rows - term_height) {
            ...
            // 🔥 லைன் நம்பர் பிரிண்ட் செய்யும் லாஜிக்
            ...
            if (line_end >= editor_len) {
                // மீதமுள்ள காலி இடத்திற்கு வெறும் லைன் நம்பர் மட்டும்
                while(row <= total_rows - term_height) {
    ...
    // 🔥 Cursor X-Position Fix (+6 added to account for line numbers)
