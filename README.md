...
- **`bdh-db`** (DB Console) — Custom database console integration built directly into the UI.
...
> Termux users can run `./install.sh` without `sudo`.
...
| Key | Action |
| :--- | :--- |
| `TAB` | Switch focus between BDH-TREE (Left), BDH-EDIT (Right), and BDH-TERMINAL (Bottom) |
| `↑ / ↓` | Navigate files in the tree |
| `Enter` | Open a directory or load a file into the editor |
| `Ctrl + S` | Save the active file |
| `Ctrl + P` | Open the integrated PostgreSQL DB Console |
| `Ctrl + T` | Focus the integrated Terminal to run shell commands |
| `+ / -` | Increase / Decrease terminal pane height |
| `< / >` | Shrink / Expand file tree pane width |
| `Q / ESC` | Quit the IDE |
...
```
bdh-linux-ide/
├── src/               # Pure C Source Code (main.c, bdh-tree.c, bdh-edit.c, bdh-db.c, bdh-ui.c)
